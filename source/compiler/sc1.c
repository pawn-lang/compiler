...
static void declglb(char* firstname,
    int firsttag,
    int fpublic,
    int fstatic,
    int stock,
...
static void initials(int ident,
    int tag,
    cell* size,
    int dim[],
    int numdim,
    constvalue_root* enumroot);
static cell initarray(int ident,
    int tag,
    int dim[],
    int numdim,
    int cur,
    int startlit,
    int counteddim[],
    constvalue_root* lastdim,
    constvalue_root* enumroot,
    int* errorfound);
static cell initvector(int ident,
    int tag,
    cell size,
    int startlit,
    int fillzero,
...
static int newfunc(char* firstname,
    int firsttag,
    int fpublic,
    int fstatic,
    int stock);
static int declargs(symbol* sym, int chkshadow);
static void doarg(char* name,
    int ident,
    int offset,
    int tags[],
    int numtags,
    ...
    va_list argptr)
{
    ...
    if (number != 0) {
        ...
        if (firstline >= 0)
            fprintf(stderr, "%s(%d -- %d) : %s %03d: ", filename, firstline, lastline,
                pre, number);
        else
    ...
    pc_compile(int argc, char* argv[])
{
   ...
   */
    ...
    if (inpf_org == NULL)
        error(100, inpfname);
    freading = TRUE;
    outf = (FILE*)pc_openasm(
        outfname); /* first write to assembler file (may be temporary) */
    if (outf == NULL)
        error(101, outfname);
    ...
    if (sc_listing) {
        ...
        sprintf(string,
            ...
            "#pragma semicolon %s\n"
            "#pragma tabsize %d\n",
            sc_ctrlchar, sc_packstr ? "true" : "false",
            sc_needsemicolon ? "true" : "false", sc_tabsize);
...
/* write a report, if requested */
    ...
    if (sc_makereport) {
        ...
        if (strlen(reportname) > 0)
            frep = fopen(reportname,
                "wb"); /* avoid translation of \n to \r\n in DOS/Windows */
        if (frep != NULL) {
   ...
   */
    ...
    sc_status = statWRITE; /* allow to write --this variable was reset by
                                      resetglobals() */
    setstringconstants();
    writeleader(&glbtab);
    setfileconst(inpfname);
...
cleanup:
    ...
    if (errnum == 0 && strlen(errfname) == 0) {
        ...
        if (flag_exceed)
            error(106,
                pc_amxlimit + pc_amxram); /* this causes a jump back to label "cleanup" */
    } /* if */
#endif
...
#endif
    int
    pc_addconstant(char* name, cell value, int tag)
{
...
static void initglobals(void)
{
    ...
    hashtable_init(&symbol_cache_ht, sizeof(symbol*), (16384 / 3 * 2),
        NULL); /* 16384 slots */
...
{
    return (*(optptr + 1) == '=' || *(optptr + 1) == ':') ? optptr + 2
                                                          : optptr + 1;
}
 ...
 */
static void parserespf(char* filename,
    char* oname,
    char* ename,
    char* pname,
    char* rname,
    ...
    for (arg = 1; arg < argc; arg++) {
        ...
        if (isoption) {
            ...
            case 'C':
                ...
                break;
            case 'c':
                strlcpy(codepage, option_value(ptr),
                    MAXCODEPAGE); /* set name of codepage */
                break;
            ...
            case 'e':
                if (ename)
                    strlcpy(ename, option_value(ptr),
            ...
            case 'o':
                if (oname)
                    strlcpy(oname, option_value(ptr),
            ...
            case 'p':
                if (pname)
                    strlcpy(pname, option_value(ptr),
            ...
            case 'r':
                if (!rname)
                    break;
                strlcpy(rname, option_value(ptr),
                    _MAX_PATH); /* set name of report file */
                sc_makereport = TRUE;
                if (strlen(rname) > 0) {
            ...
            if (i > sNAMEMAX) {
                i = sNAMEMAX;
                error(200, argv[arg],
                    sNAMEMAX); /* symbol too long, truncated to sNAMEMAX chars */
            } /* if */
            strlcpy(str, argv[arg], i + 1); /* str holds symbol name */
            i = atoi(ptr + 1);
            add_builtin_constant(str, i, sGLOBAL, 0);
        } else if (oname) {
            strlcpy(str, argv[arg],
                sizeof(str) - 2); /* -2 because default extension is ".p" */
...
/* add the default "include" directory */
     ...
     */
       ...
       * is found. This typically means that the compiler is in a "bin"
       * sub-directory and the "include" is below the *parent*. So find the
       * parent...
       */
            *base = '\0';
...
static void setcaption(void)
{
    pc_printf("Pawn compiler " VERSION_STR
...
static void about(void)
{
    if (strlen(errfname) == 0) {
        ...
        pc_printf("Options:\n");
        pc_printf(
            "         -A<num>  alignment in bytes of the data segment and "
            "the stack\n");
...
#endif
        pc_printf(
            "         -c<name> codepage name or number; e.g. 1252 for "
            "Windows Latin-1\n");
...
#if defined __WIN32__ || defined _WIN32 || defined _Windows
        pc_printf(
            "         -H<hwnd> window handle to send a notification message "
            "on finish\n");
#endif
        ...
        pc_printf(
            "         -R[+/-]  add detailed recursion report with call "
            "chains (default=%c)\n",
            pc_recursion ? '+' : '-');
#if !defined SC_LIGHT
        pc_printf(
            "         -r[name] write cross reference report to console or to "
            "specified file\n");
#endif
        ...
        pc_printf(
            ...
            sc_needsemicolon ? '+' : '-');
        pc_printf(
            "         -([+/-]  require parantheses for function invocation "
            "(default=%c)\n",
            optproccall ? '-' : '+');
        ...
        pc_printf(
            "\nOptions may start with a dash or a slash; the options \"-d0\" "
            "and \"/d0\" are\n");
        pc_printf("equivalent.\n");
#endif
        pc_printf(
            "\nOptions with a value may optionally separate the value from "
            "the option letter\n");
        pc_printf(
            "with a colon (\":\") or an equal sign (\"=\"). That is, the "
            "options \"-d0\", \"-d=0\"\n");
...
#error Unsupported cell size
    ...
    add_builtin_constant("ucharmax", (1 << (sizeof(cell) - 1) * 8) - 1, sGLOBAL,
        0);
...
static int getclassspec(int initialtok,
    int* fpublic,
    int* fstatic,
    int* fstock,
    int* fconst)
...
static void parse(void)
{
    ...
    while (freading) {
        ...
        case tSTATIC:
           ...
           * opening paranthesis of a function would be expected. To back out
           * after deciding it was a declaration of a static variable after all,
           * we have to store the symbol name and tag.
           */
                if (getclassspec(tok, &fpublic, &fstatic, &fstock, &fconst)) {
                    assert(!fpublic);
...
void sc_attachdocumentation(symbol* sym)
{
   ...
   */
    ...
    length = 0;
    for (line = 0; (str = get_docstring(line)) != NULL && *str != sDOCSEP;
         line++) {
        if (length > 0)
    ...
    int fconst)
{
    ...
    do {
        ...
        while (matchtoken('[')) {
            ...
            size = needsub(&idxtag[numdim],
                &enumroot); /* get size; size==0 for "var[]" */
#if INT_MAX < LONG_MAX
            if (size > INT_MAX)
       ...
       */
        ...
        /* we have either:
     * a) not found a matching variable (or rejected it, because it was a
     * shadow) b) found a global variable and we were looking for that global
     * variable c) found a state variable in the automaton that we were looking
     * for
     */
        assert(sym == NULL || sym->states == NULL && sc_curstates == 0 || sym->states != NULL && sym->next != NULL && sym->states->first->index == sc_curstates);
     ...
     */
        ...
        initials(ident, tag, &size, dim, numdim,
            enumroot); /* stores values in the literal queue */
        assert(size >= litidx);
        if (numdim == 1)
            dim[0] = (int)size;
     ...
     */
            ...
            if (glb_declared > 0 && (map = (unsigned char*)malloc((glb_declared + 7) / 8)) != NULL) {
                ...
                for (sweep = glbtab.next; sweep != NULL; sweep = sweep->next) {
                    if (sweep->parent != NULL || sweep->states == NULL || sweep == sym)
                        continue; /* hierarchical type, or no states, or same as this
             variable */
        ...
        if (sym == NULL) { /* define only if not yet defined */
            ...
            if (sc_curstates > 0)
              ...
              */
            assert(sym->states == NULL && sc_curstates == 0 || sym->states->first != NULL && sym->states->first->index == sc_curstates && sym->states->first->next == NULL);
            sym->addr = address;
...
static int declloc(int fstatic)
{
     ...
     */
        ...
        if (fstatic) {
            ...
            while (litidx < cur_lit + size)
                litadd(0);
            sym = addvariable(name, (cur_lit + glb_declared) * sizeof(cell), ident,
                sSTATIC, tag, dim, numdim, idxtag, nestlevel);
        } else {
            declared += (int)size; /* variables are put on stack, adjust "declared" */
            sym = addvariable(name, -declared * sizeof(cell), ident, sLOCAL, tag, dim,
                numdim, idxtag, nestlevel);
            if (ident == iVARIABLE) {
                assert(!staging);
     ...
     * to initialize it */
         ...
         * "uWRITTEN" flag that store() set */
                ...
                /* final literal values that are zero make no sense to put in the
         * literal pool, because values get zero-initialized anyway; we check
         * for this, because users often explicitly initialize strings to ""
         */
         ...
         */
           ...
           * it is, more compact code can be generated */
                    ...
                    int i;
                    for (i = cur_lit; i < litidx && litq[i] == first; i++) /* nothing */
                        ;
                    if (i == litidx) {
...
static cell calc_arraysize(int dim[], int numdim, int cur)
{
    ...
    if (newsize == 0)
        return 0;
    if ((ucell)subsize >= CELL_MAX || newsize >= CELL_MAX || newsize < (ucell)subsize || newsize * sizeof(cell) >= CELL_MAX)
        return CELL_MAX;
    return newsize;
}

static void adjust_indirectiontables(int dim[],
    int numdim,
    int startlit,
    constvalue_root* lastdim,
    int* skipdim)
 ...
 *    size==array cells (count), if 0 on input, the routine counts the number of
 * elements tag==required tagname id (not the returned tag)
 *
    ...
    constvalue_root* enumroot)
{
    ...
    if (!matchtoken('=')) {
       ...
       */
            for (tablesize = calc_arraysize(dim, numdim - 1, 0); tablesize > 0;
                 tablesize--)
                litadd(0);
    ...
    if (ident == iVARIABLE) {
       ...
       */
            ...
            initarray(ident, tag, dim, numdim, 0, curlit, counteddim, &lastdim,
                enumroot, &errorfound);
    ...
    int* errorfound)
{
     ...
     */
         ...
         */
                ...
                prev2_idx = prev1_idx;
                prev1_idx = litidx;
                dsize = initvector(ident, tag, dim[cur + 1], curlit, TRUE, enumroot,
                    errorfound);
    ...
    if (matchtoken('{')) {
       ...
       */
            ...
            if (enumfield != NULL) {
                ...
                if (cmptag > 1)
                    error(91,
                        enumfield->name); /* ambiguous constant, needs tag override */
...
static void decl_enum(int vclass, int fstatic)
{
   ...
   */
    ...
    if (strlen(enumname) > 0) {
        /* already create the root symbol, so the fields can have it as their
     * "parent" */
   ...
   */
    if (enumsym != NULL) {
        ...
        sc_attachdocumentation(
            enumsym); /* attach any documenation to the enumeration */
    } /* if */
}
    ...
    do {
        ...
        if (fsa < 0 || fsaname[0] != '\0') {
            ...
            if (fsa >= 0 && automaton->index != fsa)
                error(
                    83,
                    funcname); /* multiple automatons for a single function/variable */
            fsa = automaton->index;
        } /* if */
...
static void attachstatelist(symbol* sym, int state_id)
{
    ...
    if ((sym->usage & uDEFINE) != 0 && (sym->states == NULL || state_id == 0))
        error(21, sym->name); /* function already defined, either without states or
                         the current definition has no states */
     ...
     */
        ...
        for (stateptr = sym->states->first;
             stateptr != NULL && stateptr->index != state_id;
     ...
     */
        if (state_id == -1 && sc_status != statFIRST) {
            ...
            assert(sym->states != NULL);
            for (stateptr = sym->states->first;
                 stateptr != NULL && stateptr->index == -1; stateptr = stateptr->next)
                /* nothing */;
            if (stateptr == NULL)
...
static int operatoradjust(int opertok,
    symbol* sym,
    char* opername,
    int resulttag)
{
    ...
    while (arg = &sym->dim.arglist[count], arg->ident != 0) {
        ...
        if (arg->hasdefault)
            error(59, arg->name); /* arguments of an operator may not have a default
                         value */
        count++;
    } /* while */
   ...
   * for all other (binary) operators and the special '~' operator, count must
   * be 2
   */
    switch (opertok) {
    case '!':
    ...
    case tDEC:
        if (count != 1)
            error(62); /* number or placement of the operands does not fit the
                operator */
        break;
    case '-':
        if (count != 1 && count != 2)
...
static int check_operatortag(int opertok, int resulttag, char* opername)
{
    ...
    case tlGE:
        if (resulttag != pc_addtag("bool")) {
            error(63, opername,
                "bool:"); /* operator X requires a "bool:" result tag */
            return FALSE;
        } /* if */
        break;
    case '~':
...
SC_FUNC char* operator_symname(char* symname,
    char* opername,
    int tag1,
    int tag2,
    int numtags,
...
SC_FUNC void check_index_tagmismatch(char* symname,
    int expectedtag,
    int actualtag,
    int allowcoerce,
    int errline)
{
    ...
    if (!matchtag(expectedtag, actualtag, allowcoerce)) {
        constvalue* tagsym;
        char expected_tagname[sNAMEMAX + 3] = "none (\"_\"),",
                                         actual_tagname[sNAMEMAX + 2] = "none (\"_\")"; /* two extra characters
                             for quotes */
        if (expectedtag != 0) {
            tagsym = find_tag_byval(expectedtag);
            sprintf(expected_tagname, "\"%s\",",
                (tagsym != NULL) ? tagsym->name : "-unknown-");
        } /* if */
        if (actualtag != 0) {
            tagsym = find_tag_byval(actualtag);
            sprintf(actual_tagname, "\"%s\"",
                (tagsym != NULL) ? tagsym->name : "-unknown-");
        } /* if */
        if (errline > 0)
            errorset(sSETPOS, errline);
        error(229, symname, expected_tagname,
            actual_tagname); /* index tag mismatch */
        if (errline > 0)
            errorset(sSETPOS, -1);
    } /* if */
}

SC_FUNC void check_tagmismatch(int formaltag,
    int actualtag,
    int allowcoerce,
    int errline)
{
    if (!matchtag(formaltag, actualtag, allowcoerce)) {
        constvalue* tagsym;
        char formal_tagname[sNAMEMAX + 3] = "none (\"_\"),",
                                       actual_tagname[sNAMEMAX + 2] = "none (\"_\")"; /* two extra characters
                           for quotes */
        if (formaltag != 0) {
            tagsym = find_tag_byval(formaltag);
            sprintf(formal_tagname, "\"%s\",",
                (tagsym != NULL) ? tagsym->name : "-unknown-");
        } /* if */
        if (actualtag != 0) {
            tagsym = find_tag_byval(actualtag);
...
SC_FUNC void check_tagmismatch_multiple(int formaltags[],
    int numtags,
    int actualtag,
    int errline)
{
    if (!checktag(formaltags, numtags, actualtag)) {
        int i;
        constvalue* tagsym;
        char formal_tagnames[sLINEMAX] = "",
             actual_tagname[sNAMEMAX + 2] = "none (\"_\")";
        ...
        for (i = 0; i < numtags; i++) {
            if (formaltags[i] != 0) {
                ...
                sprintf(formal_tagnames, "%s\"%s\"", formal_tagnames,
                    (tagsym != NULL) ? tagsym->name : "-unknown-");
            } else {
                notag_allowed = TRUE;
            } /* if */
        ...
        strlcat(formal_tagnames, (numtags == 1) ? "," : ";",
        ...
        if (errline > 0)
            errorset(sSETPOS, errline);
        error(213, (numtags == 1) ? "tag" : "tags", formal_tagnames,
            actual_tagname); /* tag mismatch */
        if (errline > 0)
            errorset(sSETPOS, -1);
    } /* if */
}
...
static void check_reparse(symbol* sym)
{
   ...
   */
    if ((sym->usage & (uPROTOTYPED | uREAD)) == uREAD && sym->tag != 0) {
        ...
        sc_status = statWRITE; /* temporarily set status to WRITE, so the warning
                          isn't blocked */
        error(208);
        sc_status = curstatus;
    ...
    int stock)
{
   ...
   */
    ...
    if (strcmp(symbolname, uMAINFUNC) == 0 || strcmp(symbolname, uENTRYFUNC) == 0) {
        if (argcnt > 0)
            error(
                5); /* "main()" and "entry()" functions may not have any arguments */
   ...
   * prototype */
    if (matchtoken(';')) {
        ...
        delete_symbols(&loctab, 0, TRUE,
            TRUE); /* prototype is done; forget everything */
        return TRUE;
    } /* if */
   ...
   * in the second stage), shut code generation off */
    ...
    sym->codeaddr = code_idx;
    sc_attachdocumentation(
        sym); /* attach collected documenation to the function */
    ...
    testsymbols(&loctab, 0, TRUE,
        TRUE); /* test for unused arguments and labels */
    delete_symbols(&loctab, 0, TRUE, TRUE); /* clear local variables queue */
       ...
       * Pawn currently does not forbid them) */
            ...
            if (result) {
                if ((a1->hasdefault & uSIZEOF) != 0 || (a1->hasdefault & uTAGOF) != 0)
                    result = a1->hasdefault == a2->hasdefault && strcmp(a1->defvalue.size.symname, a2->defvalue.size.symname) == 0 && a1->defvalue.size.level == a2->defvalue.size.level;
                else
   ...
   */
        ...
        do { /* there are arguments; process them */
            ...
            case '&':
                if (ident != iVARIABLE || numtags > 0)
                    error(1, "-identifier-", "&");
                if (fconst)
                    error(238, "const reference"); /* meaningless combination of class
                                    specifiers */
                ident = iREFERENCE;
                break;
            case tCONST:
           ...
           */
                doarg(name, ident, (argcnt + 3) * sizeof(cell), tags, numtags,
                    fpublic, fconst, chkshadow, &arg);
                if (fpublic && arg.hasdefault)
                    error(59, name); /* arguments of a public function may not have a
                      default value */
                if ((sym->usage & uPROTOTYPED) == 0) {
                    /* redimension the argument list, add the entry */
                    sym->dim.arglist = (arginfo*)realloc(
                        sym->dim.arglist, (argcnt + 2) * sizeof(arginfo));
                    if (sym->dim.arglist == 0)
                        error(103); /* insufficient memory */
                    memset(&sym->dim.arglist[argcnt + 1], 0,
                        sizeof(arginfo)); /* keep the list terminated */
            ...
            case tELLIPS:
                if (ident != iVARIABLE)
                    error(10); /* illegal function or declaration */
                if (fconst)
                    error(238, "const variable arguments"); /* meaningless combination
                                             of class specifiers */
                if (numtags == 0)
                    tags[numtags++] = 0; /* default tag */
                if ((sym->usage & uPROTOTYPED) == 0) {
    ...
    for (idx = 0; idx < argcnt && arglist[idx].ident != 0; idx++) {
       ...
       */
            ...
            assert(ptr != NULL);
            for (altidx = 0;
                 altidx < argcnt && strcmp(ptr, arglist[altidx].name) != 0; altidx++)
                /* nothing */;
            if (altidx >= argcnt) {
                ...
                if (arglist[idx].defvalue.size.level > 0 && arglist[idx].defvalue.size.level >= arglist[altidx].numdim)
         ...
         */
                ...
                if (arglist[altidx].ident != iREFARRAY && (arglist[idx].hasdefault & uSIZEOF) != 0) {
                    if ((arglist[idx].hasdefault & uTAGOF) != 0) {
                        error(81,
                            arglist[idx].name); /* cannot take "tagof" an indexed array */
                    } else {
    ...
    arginfo* arg)
{
    ...
    if (matchtoken('[')) {
        ...
        do {
            ...
            size = needsub(&arg->idxtag[arg->numdim],
                &enumroot); /* may be zero here, it is a pointer anyway */
#if INT_MAX < LONG_MAX
            if (size > INT_MAX)
                error(105); /* overflow, exceeding capacity */
#endif
    ...
    if (argsym != NULL) {
        ...
        assert(numtags > 0);
        argsym = addvariable(name, offset, ident, sLOCAL, tags[0], arg->dim,
            arg->numdim, arg->idxtag, 0);
        if (fpublic)
            ...
            if (xmlparam != NULL) {
                if (strncmp(ptr, xmlparam, xmlparam_len) == 0 && (*(ptr + xmlparam_len) <= ' ' || *(ptr + xmlparam_len) == '=')) {
                    ...
                    if (*ptr == '=') {
                        ...
                        if (strncmp(ptr, xmlvalue, xmlvalue_len) == 0 && (*(ptr + xmlvalue_len) <= ' ' || *(ptr + xmlvalue_len) == '>' || *(ptr + xmlvalue_len) == '"' || *(ptr + xmlvalue_len) == '\''))
                            match = TRUE; /* found it */
                    } /* if */
                } /* if */
            } else {
...
static void make_report(symbol* root, FILE* log, char* sourcefile)
{
    ...
    fprintf(log,
        "<?xml-stylesheet href=\"file:///%s/xml/pawndoc.xsl\" "
        "type=\"text/xsl\"?>\n",
        symname);
   ...
   */
    ...
    for (sym = root->next; sym != NULL; sym = sym->next) {
        ...
        assert(sym->ident == iCONSTEXPR || sym->ident == iVARIABLE || sym->ident == iARRAY || sym->ident == iFUNCTN);
    ...
    for (sym = root->next; sym != NULL; sym = sym->next) {
        ...
        for (arg = 0; sym->dim.arglist[arg].ident != 0; arg++) {
            ...
            paraminfo = (sym->dim.arglist[arg].numtags > 1 || sym->dim.arglist[arg].tags[0] != 0) || sym->dim.arglist[arg].ident == iREFERENCE || sym->dim.arglist[arg].ident == iREFARRAY;
            if (paraminfo)
       ...
       * sym->documentation)
       */
            if (sym->documentation != NULL && find_xmltag(sym->documentation, "param", "name", sym->dim.arglist[arg].name, &outer_start, &outer_length, &inner_start, &inner_length)) {
                char* tail;
...
static void reduce_referrers(symbol* root)
{
    ...
    do {
        ...
        for (sym = root->next; sym != NULL; sym = sym->next) {
            if (sym->parent != NULL)
                continue; /* hierarchical data type */
            if (sym->ident == iFUNCTN && (sym->usage & uNATIVE) == 0 && (sym->usage & uPUBLIC) == 0 && strcmp(sym->name, uMAINFUNC) != 0 && strcmp(sym->name, uENTRYFUNC) != 0 && count_referrers(sym) == 0) {
            ...
            } else if ((sym->ident == iVARIABLE || sym->ident == iARRAY) && (sym->usage & uPUBLIC) == 0 && sym->parent == NULL && count_referrers(sym) == 0) {
                sym->usage &= ~(uREAD | uWRITTEN); /* erase usage bits if there is no referrer */
            } /* if */
        } /* for */
...
#if !defined SC_LIGHT
static long max_stacksize_recurse(symbol** sourcesym,
    symbol* sym,
    symbol** rsourcesym,
    long basesize,
    int* pubfuncparams,
    ...
    for (i = 0; i < sym->numrefers; i++) {
        if (sym->refer[i] != NULL) {
            ...
            *(rsourcesym + 1) = NULL;
            ...
            size = max_stacksize_recurse(sourcesym, sym->refer[i], rsourcesym + 1,
                sym->x.stacksize, pubfuncparams, recursion);
            if (maxsize < size)
                maxsize = size;
...
static long max_stacksize(symbol* root, int* recursion)
{
   ...
   */
    ...
    assert(root != NULL);
    assert(recursion != NULL);
    /* count number of functions (for allocating the stack for recursion
   * detection) */
    numfunctions = 0;
    for (sym = root->next; sym != NULL; sym = sym->next) {
        if (sym->ident == iFUNCTN) {
    ...
    *recursion = 0; /* assume no recursion */
    for (sym = root->next; sym != NULL; sym = sym->next) {
        ...
        size = max_stacksize_recurse(symstack, sym, rsymstack, 0L, &maxparams,
            &recursion_detected);
    ...
    while (sym != NULL && sym->compound >= level) {
        ...
        case iFUNCTN:
            if ((sym->usage & (uDEFINE | uREAD | uNATIVE | uSTOCK | uPUBLIC)) == uDEFINE) {
                funcdisplayname(symname, sym->name);
                if (strlen(symname) > 0)
                    error(203, symname); /* symbol isn't used ... (and not
                          public/native/stock) */
            } /* if */
        ...
        default:
            ...
            if ((sym->usage & (uWRITTEN | uREAD | uSTOCK | uPUBLIC)) == 0) {
                errorset(sSETPOS, sym->lnumber);
                error(203, sym->name,
                    sym->lnumber); /* symbol isn't used (and not stock) */
                errorset(sSETPOS, -1);
    ...
    while (sym != NULL && sym->compound >= level) {
        if ((sym->ident == iVARIABLE || sym->ident == iARRAY) && !(sym->vclass == sSTATIC && sym->fnumber == -1)) {
           ...
           */
                ...
                ffcall(opsym, NULL, 1);
                if (sc_status != statSKIP)
                    markusage(opsym, uREAD); /* do not mark as "used" when this call
                            itself is skipped */
...
static constvalue* insert_constval(constvalue* prev,
    constvalue* next,
    const char* name,
    cell val,
    int index)
...
SC_FUNC constvalue* find_constval(constvalue_root* table,
    char* name,
    int index)
{
   ...
   * defind constants. */
    ...
    if (sym != NULL) {
        ...
        if ((sym->usage & uENUMFIELD) != 0) {
            /* enum field, special case if it has a different tag and the new symbol
       * is also an enum field */
            constvalue* tagid;
            symbol* tagsym;
            if (sym->tag == tag)
       ...
       */
            ...
            redef = 1; /* redefinition of a constant (non-enum) to a different tag is
              not allowed */
        } /* if */
        if (redef) {
            error(21, name); /* symbol already defined */
 ...
 */
SC_FUNC symbol* add_builtin_constant(char* name,
    cell val,
    int vclass,
    int tag)
{
 ...
 */
SC_FUNC symbol* add_builtin_string_constant(char* name,
    const char* val,
    int vclass)
{
    symbol* sym;
   ...
   * in the second stage. */
    if (sc_status == statIDLE || (sc_status == statWRITE && (sym->usage & uREAD) != 0)) {
        assert(litidx == 0);
        begdseg();
        while (*val != '\0')
            litadd(*val++);
...
static void compound(int stmt_sameline, int starttok)
{
   ...
   */
    ...
    testsymbols(&loctab, nestlevel, FALSE,
        TRUE); /* look for unused block locals */
    declared = save_decl;
 ...
 *  sets the global variable "sc_intest" to true, it is restored upon
 * termination. In the case the assignment was intended, use parantheses around
 * the expression to avoid the warning; primary() sets "sc_intest" to 0.
 *
...
static int doif(void)
{
     ...
     * has a lower indent than the matching "if" */
        ...
        flab2 = getlabel();
        if ((lastst != tRETURN) && (lastst != tGOTO))
            jumplabel(flab2); /* "true" branch jumps around "else" clause, unless the
                     "true" branch statement already jumped */
...
static int dofor(void)
{
   ...
   */
    ...
    if (matchtoken(';')) {
        ...
        endlessloop = test(wq[wqEXIT], TEST_PLAIN,
...
static void doswitch(void)
{
    ...
    endtok = matchtoken('(') ? ')' : tDO;
    doexpr(TRUE, FALSE, FALSE, FALSE, NULL, NULL,
        TRUE); /* evaluate switch expression */
    needtoken(endtok);
        ...
        case tCASE:
           ...
           */
                for (csp = NULL, cse = caselist.first;
                     cse != NULL && cse->value < val; csp = cse, cse = cse->next)
                    /* nothing */;
...
static void doassert(void)
{
    ...
    if ((sc_debug & sCHKBOUNDS) != 0) {
        ...
        test(flab1, TEST_PLAIN,
            TRUE); /* get expression and branch to flab1 if true */
...
static void SC_FASTCALL emit_invalid_token(int expected_token,
    int found_token)
{
    char s[2];
    extern char* sc_tokens[];
    ...
    if (found_token < tFIRST) {
        ...
        error(1, sc_tokens[expected_token - tFIRST],
            sc_tokens[found_token - tFIRST]);
    } /* if */
}

static int SC_FASTCALL emit_param_any_internal(emit_outval* p,
    int expected_tok,
    int allow_nonint,
    int allow_expr)
{
...
static void SC_FASTCALL emit_param_index(emit_outval* p,
    int isrange,
    const cell* valid_values,
    int numvalues)
{
    int i;
    cell val;
...
static void SC_FASTCALL emit_parm1_shift(char* name)
{
    ...
    emit_param_index(&p[0], TRUE, valid_values,
        (sizeof valid_values / sizeof valid_values[0]));
    outinstr(name, p, (sizeof p / sizeof p[0]));
}
...
static void SC_FASTCALL emit_do_lodb_strb(char* name)
{
    ...
    emit_param_index(&p[0], FALSE, valid_values,
        (sizeof valid_values / sizeof valid_values[0]));
    outinstr(name, p, (sizeof p / sizeof p[0]));
}
...
static void SC_FASTCALL emit_do_pushn_c(char* name)
{
    ...
    assert(name[0] == 'p' && name[1] == 'u' && name[2] == 's' && name[3] == 'h' && '2' <= name[4] && name[4] <= '5');
    numargs = name[4] - '0';
    for (i = 0; i < numargs; i++)
        emit_param_any(&p[i]);
...
static void SC_FASTCALL emit_do_pushn_s_adr(char* name)
{
    ...
    assert(name[0] == 'p' && name[1] == 'u' && name[2] == 's' && name[3] == 'h' && '2' <= name[4] && name[4] <= '5' && name[5] == '.');
    numargs = name[4] - '0';
    for (i = 0; i < numargs; i++)
...
SC_FUNC void emit_parse_line(void)
{
     ...
     */
        lptr -= len;
        for (i = 0; i < sizeof(name) && (isalnum(*lptr) || *lptr == '.');
             ++i, ++lptr)
    ...
    if (!matchtoken(tTERM)) {
        ...
        if (ident == iARRAY || ident == iREFARRAY) {
           ...
           */
                    ...
                    /* the address of the array is stored in a hidden parameter; the
           * address of this parameter is 1 + the number of parameters (times
           * the size of a cell) + the size of the stack frame and the return
           * address base + 0*sizeof(cell)         == previous "base" base +
           * 1*sizeof(cell)         == function return address base +
           * 2*sizeof(cell)         == number of arguments base + 3*sizeof(cell)
           * == first argument of the function
           *   ...
           *   base + ((n-1)+3)*sizeof(cell) == last argument of the function
           *   base + (n+3)*sizeof(cell)     == hidden parameter with array
           * address
           */
                    assert(curfunc != NULL);
                    assert(curfunc->dim.arglist != NULL);
                    for (argcount = 0; curfunc->dim.arglist[argcount].ident != 0;
                         argcount++)
                        /* nothing */;
                    sub = addvariable(curfunc->name, (argcount + 3) * sizeof(cell),
                        iREFARRAY, sGLOBAL, curfunc->tag, dim, numdim,
                        idxtag, 0);
                    sub->parent = curfunc;
                    if (curfunc)
                        curfunc->child = sub;
        ...
        if ((doc = (char*)malloc(length * sizeof(char))) != NULL) {
            do {
                ...
                if (get_autolist(0) != NULL) {
                    ...
                    for (index = 0; (str = get_autolist(index)) != NULL; index++) {
                        /* remove the ')' token that may be appended before detecting that
