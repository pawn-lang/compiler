...
static stringpair* insert_stringpair(stringpair* root,
    char* first,
    char* second,
    int matchlength)
{
...
static stringpair* find_stringpair(stringpair* cur,
    char* first,
    int matchlength)
{
    int result = 0;
...
static stringlist* insert_string(stringlist* list, char* string)
{
    ...
    if (list->data == NULL) {
        ...
        *list = newlist;
    } else if (list->length == list->size || list->length == list->data + list->size - list->strings) {
...
/* ----- include paths list -------------------------------------- */
static stringlist includepaths = { NULL, NULL, 0,
    0 }; /* directory list for include files */
...
static stringpair substpair = { NULL, NULL,
    NULL }; /* list of substitution pairs */
...
SC_FUNC stringpair* insert_subst(char* pattern,
    char* substitution,
    int prefixlen)
{
    stringpair* cur;
...
SC_FUNC valuepair* push_heaplist(long first, long second)
{
    ...
    for (last = &heaplist; last->next != NULL; last = last->next) /* nothing */
        ;
    last->next = cur;
    return cur;
}
    ...
    if (sc_status == statWRITE && (sc_debug & sSYMBOLIC) != 0) {
        ...
        if (sym->ident == iFUNCTN) {
            sprintf(string, "S:%" PRIxC " %x:%s %" PRIxC " %" PRIxC " %x %x",
                sym->addr, sym->tag, symname, sym->addr, sym->codeaddr,
                sym->ident, sym->vclass);
        } else {
            sprintf(string, "S:%" PRIxC " %x:%s %" PRIxC " %" PRIxC " %x %x",
                sym->addr, sym->tag, symname, sym->codeaddr, code_idx, sym->ident,
                sym->vclass);
        } /* if */
