...
static OPCODE opcodelist[] = {
    ...
    { 135, "sysreq.n", sIN_CSEG,
        parm2 }, /* version 9 (replaces SYSREQ.d from earlier version) */
...
SC_FUNC int assemble(FILE* fout, FILE* fin)
{
    ...
    if (setjmp(compact_err) != 0) {
        assert(
            sc_compress); /* cannot arrive here if compact encoding was disabled */
        sc_compress = FALSE;
        pc_resetbin(fout, 0);
   ...
   */
    ...
    for (sym = glbtab.next; sym != NULL; sym = sym->next) {
        ...
        if (sym->ident == iFUNCTN) {
            ...
            if ((sym->usage & uPUBLIC) != 0 && ((sym->usage & uSTOCK) == 0 || (sym->usage & (uREAD | uWRITTEN)) != 0))
                match = ++numpubvars;
        } /* if */
        if (match) {
    ...
    numlibraries = 0;
    if (pc_addlibtable) {
        for (constptr = libname_tab.first; constptr != NULL;
    ...
    numtags = 0;
    for (constptr = tagname_tab.first; constptr != NULL;
         constptr = constptr->next) {
   ...
   * => since the code segment is padded to a sc_dataalign boundary, the data
   * segment is aligned
   ...
   */
    ...
    hdr.file_version = (char)((pc_optimize <= sOPTIMIZE_NOMACRO) ? MAX_FILE_VER_JIT
                                                                 : CUR_FILE_VERSION);
    hdr.amx_version = (char)((pc_optimize <= sOPTIMIZE_NOMACRO) ? MIN_AMX_VER_JIT
                                                                : MIN_AMX_VERSION);
     ...
     * if there are no labels */
         ...
         * whitespace character) */
                ...
                if (opcodelist[i].segment == sIN_CSEG)
                    codeindex += opcodelist[i].func(NULL, skipwhitespace(params),
                        opcodelist[i].opcode);
            } /* if */
        } /* while */
    } /* if */
...
static void append_dbginfo(FILE* fout)
{
    ...
    /* automaton table */
    for (constptr = sc_automaton_tab.first; constptr != NULL;
         constptr = constptr->next) {
    ...
    /* state table */
    for (constptr = sc_state_tab.first; constptr != NULL;
         constptr = constptr->next) {
        assert(strlen(constptr->name) > 0);
        dbghdr.states++;
