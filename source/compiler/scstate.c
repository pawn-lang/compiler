...
static statelist statelist_tab = { NULL, NULL, 0, 0,
    0 }; /* state combinations table */
...
{
    constvalue* ptr;
    for (ptr = sc_automaton_tab.first; ptr != NULL && ptr->index != id;
         ptr = ptr->next)
        /* nothing */;
    return ptr;
}
...
{
    constvalue* ptr;
    for (ptr = sc_state_tab.first; ptr != NULL && ptr->value != id;
         ptr = ptr->next)
        /* nothing */;
    return ptr;
}

SC_FUNC void state_buildlist(int** list,
    int* listsize,
    int* count,
    int stateid)
{
    ...
    for (idx = 0; idx < *count && (*list)[idx] < stateid; idx++) /* nothing */
        ;
    if (idx < *count)
        memmove(&(*list)[idx + 1], &(*list)[idx],
            (int)((*count - idx + 1) * sizeof(int)));
    (*list)[idx] = stateid;
    *count += 1;
}
    ...
    *last = 0;
    ...
    while (ptr != NULL) {
        ...
        if (ptr->fsa == fsa && ptr->numstates == count) {
            /* compare all states */
            for (i = 0; i < count && ptr->states[i] == list[i]; i++) /* nothing */
                ;
            if (i == count)
                return ptr;
        } /* if */
...
static statelist* state_getlist_ptr(int listid)
{
    ...
    assert(listid > 0);
    for (ptr = statelist_tab.next; ptr != NULL && ptr->listid != listid;
         ptr = ptr->next)
        /* nothing */;
    return ptr;
}
...
/* This function searches whether one of the states in the list of statelist
 * id's of a symbol exists in any other statelist id's of the same function; it
 * also verifies that all definitions of the symbol are in the same automaton.
 */
