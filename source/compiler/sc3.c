...
static int skim(int* opstr,
    void (*testfunc)(int),
    int dropval,
    int endval,
    int (*hier)(value*),
    value* lval);
static void dropout(int lvalue,
    void (*testfunc)(int val),
    int exit1,
    value* lval);
static int plnge(int* opstr,
    int opoff,
    int (*hier)(value* lval),
    value* lval,
    char* forcetag,
...
SC_FUNC int check_userop(void (*oper)(void),
    int tag1,
    int tag2,
    int numparam,
    value* lval,
    int* resulttag)
{
   ...
   */
    ...
    if (sym == NULL /*|| (sym->usage & uDEFINE)==0*/) { /* ??? should not check
                                                         uDEFINE; first pass
                                                         clears these bits */
    ...
    if ((sym->usage & uMISSING) != 0 || (sym->usage & uPROTOTYPED) == 0) {
        char
            symname[2 * sNAMEMAX + 16]; /* allow space for user defined operators */
    ...
    value* lval)
{
    ...
    for (;;) {
        ...
        if (foundop) {
            ...
            dropout(lvalue, testfunc, droplab,
                lval); /* found at least one operator! */
            ldconst(endval, sPRI);
 ...
 */
static int plnge_rel(int* opstr,
    int opoff,
    int (*hier)(value* lval),
    value* lval)
{
    ...
    value* lval2)
{
     ...
     */
        if ((lval1->sym != NULL && (lval1->sym->ident == iFUNCTN || lval1->sym->ident == iREFFUNC)) || (lval2->sym != NULL && (lval2->sym->ident == iFUNCTN || lval2->sym->ident == iREFFUNC)))
            pc_sideeffect = FALSE;
...
SC_FUNC int expression(cell* val,
    int* tag,
    symbol** symptr,
    int chkfuncresult)
{
...
SC_FUNC int sc_getstateid(constvalue** automaton, constvalue** state)
{
     ...
     * going (an "unknown automaton" error may occur when the "state"
     * instruction precedes any state definition)
     */
        if (!needtoken(tSYMBOL))
            return 0;
        tokeninfo(&val,
            &str); /* do not copy the name yet, must check automaton first */
        if (*automaton == NULL) {
...
static int hier14(value* lval1)
{
    ...
    cell arrayidx1[sDIMEN_MAX],
        arrayidx2[sDIMEN_MAX]; /* last used array indices */
    cell* org_arrayidx;
   ...
   * and then the general */
    ...
    lval1->arrayidx = org_arrayidx; /* restore array index pointer */
    if (lval1->ident == iARRAYCELL || lval1->ident == iARRAYCHAR || lval1->ident == iARRAY || lval1->ident == iREFARRAY) {
        ...
        if (oper)
            popreg(sALT);
        if (!oper && lval3.arrayidx != NULL && lval2.arrayidx != NULL && lval3.ident == lval2.ident && lval3.sym == lval2.sym) {
            int same = TRUE;
   ...
   */
    leftarray = lval3.ident == iARRAY || lval3.ident == iREFARRAY || ((lval3.ident == iARRAYCELL || lval3.ident == iARRAYCHAR) && lval3.constval > 1 && lval3.sym->dim.array.level == 0 && !oper && (lval2.ident == iARRAY || lval2.ident == iREFARRAY));
    if (leftarray) {
     ...
     */
        ...
        if ((lval3.ident == iARRAYCELL || lval3.ident == iARRAYCHAR) && lval3.constval > 0 && lval3.sym->dim.array.level == 0) {
            ltlength = (int)lval3.constval;
        } /* if */
        if (lval2.ident != iARRAY && lval2.ident != iREFARRAY && (lval2.sym == NULL || lval2.constval <= 0))
       ...
       */
        ...
        if (lval3.sym->dim.array.level != level)
            ...
            check_index_tagmismatch(
                (lval2.sym != NULL) ? lval2.sym->name : lval3.sym->name,
                lval3.sym->x.tags.index, idxtag, TRUE, 0);
        if (level > 0) {
       ...
       */
         ...
         */
                if (sym1->dim.array.length != sym2->dim.array.length)
                    ...
                    check_index_tagmismatch(sym2->name, sym1->x.tags.index,
                        sym2->x.tags.index, TRUE, 0);
            } /* for */
    ...
    if (!oper)
        check_tagmismatch(lval3.tag, lval2.tag, TRUE,
            -1); /* tagname mismatch (if "oper", warning
                              already given in plunge2()) */
    if (lval3.sym)
...
static int hier13(value* lval)
{
    ...
    if (matchtoken('?')) {
        ...
        check_tagmismatch(
            lval->tag, lval2.tag, FALSE,
            -1); /* tagname mismatch ('true' and 'false' expressions) */
        setlabel(flab2);
        if (sc_status == statFIRST) {
       ...
       */
        ...
        pc_sideeffect = FALSE; /* assume any of the sub-expressions had no side
                            * effect, so that a warning is given if the result
                            * of the expression is unused */
...
#error Unsupported cell size
    ...
    case tDEFINED:
        ...
        if (sym != NULL && sym->ident != iFUNCTN && sym->ident != iREFFUNC && (sym->usage & uDEFINE) == 0)
            sym = NULL; /* symbol is not a function, it is in the table, but not
                       "defined" */
        val = (sym != NULL);
    ...
    case tSIZEOF:
        ...
        if (sym->ident == iCONSTEXPR)
            ...
            return error(17, st); /* undefined symbol (symbol is in the table, but
                                 it is "used" only) */
        clear_value(lval);
        lval->ident = iCONSTEXPR;
        ...
        if (sym->ident == iARRAY || sym->ident == iREFARRAY) {
            ...
            if (level > sym->dim.array.level + 1)
                ...
                lval->constval = (idxsym != NULL && idxsym->dim.array.length > 0)
                    ? idxsym->dim.array.length
                    : 1;
            else
...
static int hier1(value* lval1)
{
    ...
    char close;
    symbol* sym;
    symbol dummysymbol, *cursym; /* for changing the index tags in case of
                                  enumerated pseudo-arrays */
...
#error Unsupported cell size
                    ...
                    /* character index */
                    if (lval2.constval < 0 || (sym->dim.array.length != 0 && sym->dim.array.length * ((8 * sizeof(cell)) / sCHARBITS) <= (ucell)lval2.constval))
                        error(32, sym->name); /* array index out of bounds */
                    if (lval2.constval != 0) {
/* don't add offsets for zero subscripts */
#if sCHARBITS == 16
       ...
       */
            if (lval2.ident == iCONSTEXPR && lval2.sym != NULL && lval2.sym->dim.array.length > 0 && sym->dim.array.level == 0) {
                lval1->tag = lval2.sym->x.tags.index;
                lval1->constval = lval2.sym->dim.array.length;
                if (lval2.tag == sym->x.tags.index && lval1->constval > 1 && matchtoken('[')) {
                    /* an array indexed with an enumeration field may be considered a
           * sub-array */
                    lexpush();
                    lvalue = FALSE; /* for now, a iREFARRAY is no lvalue */
                    lval1->ident = iREFARRAY;
 ...
 *
 *  Global references: sc_intest  (may be altered, but restored upon
 * termination)
 */
static int primary(value* lval)
{
...
static void setdefarray(cell* string,
    cell size,
    cell array_sz,
    cell* dataaddr,
    int fconst)
    ...
    int matchparanthesis)
{
    ...
    int locheap;
    int close, lvalue;
    int argpos; /* index in the output stream (argpos==nargs if positional
                     parameters) */
       ...
       */
         ...
         */
                 ...
                 * non-const "variable argument list" as a constant here */
                    ...
                    } else if (lval.ident == iCONSTEXPR || lval.ident == iEXPRESSION || lval.ident == iARRAYCHAR) {
               ...
               * value (already in PRI) there */
                    ...
                    check_tagmismatch_multiple(arg[argidx].tags, arg[argidx].numtags,
                        lval.tag, -1);
                    if (lval.tag != 0)
                        append_constval(&taglst, arg[argidx].name, lval.tag, 0);
                    break;
                case iVARIABLE:
                    if (lval.ident == iLABEL || lval.ident == iFUNCTN || lval.ident == iREFFUNC || lval.ident == iARRAY || lval.ident == iREFARRAY)
                    ...
                    check_userop(NULL, lval.tag, arg[argidx].tags[0], 2, NULL,
                        &lval.tag);
                    check_tagmismatch_multiple(arg[argidx].tags, arg[argidx].numtags,
                        lval.tag, -1);
                    if (lval.tag != 0)
                ...
                case iREFERENCE:
                    ...
                    break;
                case iREFARRAY:
                    if (lval.ident != iARRAY && lval.ident != iREFARRAY && lval.ident != iARRAYCELL) {
                        error(35, argidx + 1); /* argument type mismatch */
                        break;
                    } /* if */
                    if (lval.sym != NULL && (lval.sym->usage & uCONST) != 0 && (arg[argidx].usage & uCONST) == 0)
             ...
             * An iARRAYCELL parameter is also assumed to have a single
             * dimension.
             */
                     ...
                     */
                                    if ((lval.constval > 0 && arg[argidx].dim[0] != lval.constval) || (lval.constval < 0 && arg[argidx].dim[0] < -lval.constval))
                                        error(47); /* array sizes must match */
                                } /* if */
                            } /* if */
                        } /* if */
               ...
               */
                        while (sym->dim.array.level > 0) {
                            ...
                            if (arg[argidx].dim[level] != 0 && sym->dim.array.length != arg[argidx].dim[level])
                                ...
                                check_index_tagmismatch(sym->name, arg[argidx].idxtag[level],
                                    sym->x.tags.index, TRUE, 0);
     ...
     */
        ...
        if (arg[argidx].hasdefault) {
            if (arg[argidx].ident == iREFARRAY) {
                ...
                /* keep the lengths of all dimensions of a multi-dimensional default
         * array */
                ...
                assert(arg[argidx].numtags > 0);
                check_userop(NULL, arg[argidx].defvalue_tag, arg[argidx].tags[0], 2,
                    NULL, &dummytag);
   ...
   */
    for (argidx = 0; arg[argidx].ident != 0 && arg[argidx].ident != iVARARGS;
       ...
       */
            ...
            if (asz != NULL) {
                array_sz = asz->value;
                if (array_sz == 0)
                    error(224, arg[argidx].name); /* indeterminate array size in "sizeof"
                                           expression */
            } else {
                array_sz = 1;
            } /* if */
    ...
    {
        long totalsize;
        totalsize = declared + decl_heap + 1; /* local variables & return value
                                           * size, +1 for PROC opcode */
...
static int commutative(void (*oper)())
{
    return oper == ob_add || oper == os_mult || oper == ob_eq || oper == ob_ne || oper == ob_and || oper == ob_xor || oper == ob_or;
}
...
static int constant(value* lval)
{
    ...
    if (tok == tSYMBOL && (sym = findconst(st, &cmptag)) != 0) {
        if (cmptag > 1)
            error(91, sym->name); /* ambiguity: multiple matching constants (different
                               tags) */
        ...
        lval->constval = val - litidx; /* constval == the negative value of the
                                    * size of the literal array; using a
                                    * negative value distinguishes between
                                    * literal arrays and literal strings (this
                                    * was done for array assignment). */
    } else if (tok == '{') {
        int tag, lasttag = -1;
        val = litidx;
        do {
