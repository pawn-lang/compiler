       ...
       */
            ...
            for (state = sc_state_tab.first; state != NULL; state = state->next) {
                if (state->index == fsa_id) {
                    ...
                    for (stlist = sym->states->first; stlist != NULL;
                         stlist = stlist->next) {
                    ...
                    if (stlist == NULL && strtol(lbl_default, NULL, 16) == lbl_nostate)
                        error(230, state->name,
                            sym->name); /* unimplemented state, no fallback */
...
SC_FUNC void setline(int chkbounds)
{
     ...
     * because earlier versions of Small/Pawn have an incompatible version of
     * the line opcode
     */
        stgwrite("\tbreak\t; ");
        outval(code_idx, TRUE);
...
SC_FUNC void ffcall(symbol* sym, const char* label, int numargs)
{
    ...
    if ((sym->usage & uNATIVE) != 0) {
        ...
        stgwrite("\n"); /* write on a separate line, to mark a sequence point for
        ...
        if (sc_asmfile && (label != NULL || (!isalpha(sym->name[0]) && sym->name[0] != '_' && sym->name[0] != sc_ctrlchar))) {
            stgwrite("\t; ");
            stgwrite(symname);
        } /* if */
        stgwrite("\n");
...
SC_FUNC void setstk(cell value)
{
    ...
    if (value < 0) {
        ...
        code_idx += opcodes(1) + opargs(1);
        // ??? write zeros in the space between STK and the value in PRI (the new
        // stk)
