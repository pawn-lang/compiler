...
SC_FUNC int cp_set(const char* name)
{
    ...
    if (fp == NULL) {
        /* try opening the file in the "root path" for codepages, with a ".txt"
     * extension */
    ...
    if (fp == NULL) {
        /* try opening the file in the "root path" for codepages, with "cp" prefixed
     * before the name */
...
SC_FUNC cell cp_translate(const unsigned char* string,
...
#if !defined NO_UTF8
SC_FUNC cell get_utf8_char(const unsigned char* string,
    const unsigned char** endptr)
{
    int follow = 0;
    long lowmark = 0;
