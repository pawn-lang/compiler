...
typedef cell (*OPCODE_PROC)(FILE* ftxt,
    const cell* params,
    cell opcode,
    cell cip);
...
{
    fprintf(ftxt, "%08" PRIxC "  %s ", cip,
        opcodelist[(int)(opcode & 0x0000ffff)].name);
}
...
cell parm5(FILE* ftxt, const cell* params, cell opcode, cell cip)
{
    print_opcode(ftxt, opcode, cip);
    fprintf(ftxt,
        "%08" PRIxC " %08" PRIxC " %08" PRIxC " %08" PRIxC " %08" PRIxC "\n",
...
int main(int argc, char* argv[])
{
    ...
    if ((amxhdr.flags & AMX_FLAG_COMPACT) != 0)
        expand((unsigned char*)code, amxhdr.size - amxhdr.cod,
            amxhdr.hea - amxhdr.cod);
    ...
    while (((unsigned char*)cip - (unsigned char*)code) < codesize) {
        if (count == 0) {
            ...
            fprintf(fplist, "\n%08" PRIxC "  ",
                (cell)((cip - code) * sizeof(cell) - (amxhdr.dat - amxhdr.cod)));
        } /* if */
